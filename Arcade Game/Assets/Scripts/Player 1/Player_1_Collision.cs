...
public class Player_1_Collision : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ammo > 0 && haveGun == true)
        {
            ...
            gun.GetComponent<Rigidbody2D>().transform.position = new Vector3(transform.position.x + (direction_multiplier * 1), transform.position.y, transform.position.z);
            throwGun = false;
        }
        ...
        // Shooting
        if (Input.GetKey(KeyCode.G) && haveGun == true && gunCooldown <= 0)
        {
        ...
        if (Input.GetKeyDown(KeyCode.H))
        {
            ...
            GameObject throwable = Instantiate(throwablePrefab,
                new Vector3(transform.position.x, transform.position.y+1, transform.position.z), Quaternion.identity) as GameObject;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "Explosion")
        {
            ...
            dir = -dir.normalized;
            rb.velocity = new Vector2(dir.x * 50, dir.y * 50);
        }
